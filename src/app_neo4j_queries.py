...
def get_activity_was_generated_by(neo4j_driver, uuid):
    ...
    query = (
        "MATCH (e:Entity)-[:WAS_GENERATED_BY]->(a:Activity) "
        "WHERE e.uuid = $uuid "
...
def get_activity(neo4j_driver, uuid):
    ...
    query = "MATCH (e:Activity) " "WHERE e.uuid = $uuid " f"RETURN e AS {record_field_name}"
...
def get_entities_by_type(neo4j_driver, entity_type, property_key=None):
    ...
    if property_key:
        ...
        _activity_query_part = schema_neo4j_queries.activity_query_part(for_all_match=True)
        query = f"MATCH (t:{entity_type}) " f"{_activity_query_part} {record_field_name}"
...
def dataset_has_component_children(neo4j_driver, dataset_uuid):
    ...
    """
    query = (
        "MATCH p=(ds1:Dataset)-[:WAS_GENERATED_BY]->(a:Activity)-[:USED]->(ds2:Dataset) "
...
def get_ancestor_organs(neo4j_driver, entity_uuid):
    ...
    query = (
        "MATCH (e:Entity {uuid: $entity_uuid})-[*]->(organ:Sample {sample_category: 'Organ'}) "
        # COLLECT() returns a list
        # apoc.coll.toSet() reruns a set containing unique nodes
...
def create_entity(neo4j_driver, entity_type, entity_data_dict, superclass=None):
    ...
    query = (
        f"CREATE (e{labels}) " f"SET e = {parameterized_str} " f"RETURN e AS {record_field_name}"
    )
...
def create_multiple_samples(
    neo4j_driver, samples_dict_list, activity_data_dict, direct_ancestor_uuid
):
    """
...
def get_ancestors(
    ...
    if is_filtered:
        query = (
            "MATCH (e:Entity)-[:USED|WAS_GENERATED_BY*]->(t:Entity) "
    ...
    with neo4j_driver.session() as session:
        record = session.read_transaction(
            _execute_readonly_tx, query, uuid=uuid, data_access_level=data_access_level
        )
...
def get_descendant_datasets(neo4j_driver, uuid, property_key=None):
    ...
    if property_key:
        query = (
            "MATCH (e:Entity)<-[:USED|WAS_GENERATED_BY*]-(descendant:Dataset) "
            # The target entity can't be a Lab
        ...
        _activity_query_part = schema_neo4j_queries.activity_query_part(for_all_match=True)
        query = (
            "MATCH (e:Entity)<-[:USED|WAS_GENERATED_BY*]-(t:Dataset) "
            # The target entity can't be a Lab
            "WHERE e.uuid = $uuid AND e.entity_type <> 'Lab' "
            f"{_activity_query_part} {record_field_name}"
        )
...
def get_ancestors_by_type(neo4j_driver, uuid, ancestor_type, property_keys=None):
    ...
    query = (
        "MATCH (e:Entity)-[:USED|WAS_GENERATED_BY*]->(a:Entity) "
    ...
    with neo4j_driver.session() as session:
        record = session.read_transaction(
            _execute_readonly_tx, query, uuid=uuid, ancestor_type=ancestor_type
        )
...
def get_descendants_by_type(neo4j_driver, uuid, descendant_type, property_keys=None):
    ...
    query = (
        "MATCH (e:Entity)<-[:USED|WAS_GENERATED_BY*]-(d:Entity) "
    ...
    with neo4j_driver.session() as session:
        record = session.read_transaction(
            _execute_readonly_tx, query, uuid=uuid, descendant_type=descendant_type
        )
...
def get_source_samples(neo4j_driver, uuid, property_keys=None):
    ...
    query = (
        "MATCH (d:Dataset)-[:WAS_GENERATED_BY|USED*]->(s:Sample) "
        "WHERE d.uuid = $uuid "
        f"RETURN {return_statement} AS {record_field_name}"
    )
...
def get_source_organ_count(neo4j_driver, uuid: str, organ: str, case_uuid: str = None):
    ...
    with neo4j_driver.session() as session:
        record = session.read_transaction(
            _execute_readonly_tx, query, uuid=uuid, organ=organ, case_uuid=case_uuid
        )
...
def get_sorted_revisions(neo4j_driver, uuid):
    ...
    query = (
        "MATCH (prev:Dataset)<-[:REVISION_OF *0..]-(e:Dataset)<-[:REVISION_OF *0..]-(next:Dataset) "
        "WHERE e.uuid=$uuid "
        # COLLECT() returns a list
        # apoc.coll.toSet() reruns a set containing unique nodes
...
def get_sorted_multi_revisions(neo4j_driver, uuid, fetch_all=True, property_key=False):
    ...
    results = []
    match_case = (
        "" if fetch_all is True else 'AND prev.status = "Published" AND next.status = "Published" '
    )
    ...
    with neo4j_driver.session() as session:
        ...
        if record and record[record_field_name] and len(record[record_field_name]) > 0:
            record[record_field_name][
                0
            ].pop()  # the target will appear twice, pop it from the next list
            if property_key:
                return record[record_field_name]
            else:
                for collection in record[record_field_name]:  # two collections: next, prev
                    ...
                    for (
                        ...
                        collection
                    ):  # each collection list contains revision lists, so 2 dimensional array
                        # Convert the list of nodes to a list of dicts
                ...
                for node in item.nodes:
                    if node["entity_type"] == "Sample":
                        if not node["uuid"] in dataset_sample_list:
                            dataset_sample_list[node["uuid"]] = {
                                "sample_category": node["sample_category"]
                            }
...
def get_previous_multi_revisions(neo4j_driver, uuid, property_key=None):
    ...
    collect_prop = f".{property_key}" if property_key else ""
    query = (
        "MATCH p=(e:Entity)-[:REVISION_OF*]->(prev:Entity) "
        "WHERE e.uuid = $uuid "
...
def get_next_multi_revisions(neo4j_driver, uuid, property_key=None):
    ...
    collect_prop = f".{property_key}" if property_key else ""
    query = (
        "MATCH n=(e:Entity)<-[:REVISION_OF*]-(next:Entity) "
        "WHERE e.uuid = $uuid "
...
def get_previous_revisions(neo4j_driver, uuid, property_key=None):
    ...
    if property_key:
        query = (
            f"MATCH (e:Entity)-[:REVISION_OF*]->(prev:Entity) "
            f"WHERE e.uuid = $uuid "
            # COLLECT() returns a list
            # apoc.coll.toSet() reruns a set containing unique nodes
...
def get_next_revisions(neo4j_driver, uuid, property_key=None):
    ...
    if property_key:
        query = (
            f"MATCH (e:Entity)<-[:REVISION_OF*]-(next:Entity) "
            f"WHERE e.uuid = $uuid "
            # COLLECT() returns a list
            # apoc.coll.toSet() reruns a set containing unique nodes
...
def get_provenance(neo4j_driver, uuid, depth, return_descendants=None, data_access_level=None):
    ...
    if data_access_level:
        ...
        predicate = (
            "MATCH(allowedEntity:Entity)-[:WAS_GENERATED_BY]->(allowedActivity:Activity) "
    ...
    with neo4j_driver.session() as session:
        return session.read_transaction(
            _execute_readonly_tx, query, uuid=uuid, depth=depth, data_access_level=data_access_level
        )
...
def get_dataset_latest_revision(neo4j_driver, uuid, public=False):
    ...
    if public:
        ...
        query = (
            "MATCH (e:Dataset)<-[:REVISION_OF*]-(next:Dataset) "
...
def get_dataset_revision_number(neo4j_driver, uuid):
    ...
    # Binding a variable length relationship pattern to a variable ('r') is deprecated
    query = (
        f"MATCH (e:Dataset)-[:REVISION_OF*]->(prev:Dataset) "
        f"WHERE e.uuid = $uuid "
...
def get_associated_organs_from_dataset(neo4j_driver, dataset_uuid):
    ...
    results = []
    query = (
        "MATCH (ds:Dataset)-[*]->(organ:Sample {sample_category: 'Organ'}) "
...
def get_associated_samples_from_dataset(neo4j_driver, dataset_uuid):
    ...
    query = (
        "MATCH (ds:Dataset)-[*]->(sample:Sample) "
    ...
    with neo4j_driver.session() as session:
        record = session.read_transaction(
            schema_neo4j_queries.execute_readonly_tx, query, dataset_uuid=dataset_uuid
        )
...
def get_associated_sources_from_dataset(neo4j_driver, dataset_uuid):
    ...
    # specimen_type -> sample_category 12/15/2022
    query = (
        "MATCH (ds:Dataset)-[*]->(source:Source) "
        "WHERE ds.uuid = $dataset_uuid "
...
def get_prov_info(neo4j_driver, param_dict, published_only):
    ...
    if "group_uuid" in param_dict:
        group_uuid_query_string = (
            f" AND toUpper(ds.group_uuid) = '{param_dict['group_uuid'].upper()}'"
        )
    ...
    if "dataset_status" in param_dict:
        dataset_status_query_string = (
            f" AND toUpper(ds.status) = '{param_dict['dataset_status'].upper()}'"
        )
    if published_only:
        published_only_query_string = " AND toUpper(ds.status) = 'PUBLISHED'"
    query = (
        f"MATCH (ds:Dataset)-[:WAS_GENERATED_BY]->(a)-[:USED]->(firstSample:Sample)-[*]->(source:Source)"
...
def create_multiple_datasets(
    neo4j_driver, datasets_dict_list, activity_data_dict, direct_ancestor_uuid
):
    """
...
def get_siblings(neo4j_driver, uuid, status, prop_key, include_revisions):
    ...
    with neo4j_driver.session() as session:
        record = session.read_transaction(
            schema_neo4j_queries.execute_readonly_tx,
            query,
            status=status,
