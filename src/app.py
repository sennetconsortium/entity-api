...
from schema.schema_constants import CacheTypeEnum, SchemaConstants
...
def get_ancestor_organs(id):
    ...
    complete_entities_list = schema_manager.get_complete_entities_list(token, organs, properties_to_skip, None)
...
def build_nodes(raw_provenance_dict, normalized_provenance_dict, token):
    for node_dict in raw_provenance_dict['nodes']:
        ...
        if (node_dict['label'] == 'Entity') and (node_dict['entity_type'] != 'Lab'):
            ...
            complete_entity_dict = schema_manager.get_complete_entity_result(token, node_dict, properties_to_skip, None)
...
def get_entities_by_type(entity_type):
    ...
    # Return all the details if no property filtering
        ...
        complete_entities_list = schema_manager.get_complete_entities_list(token, entities_list, properties_to_skip, None)
...
def create_entity(entity_type: str, user_token: str, json_data_dict: dict):
    ...
    cache_type = CacheTypeEnum.COMPLETE if len(properties_to_skip) == 0 else None
    complete_dict = schema_manager.get_complete_entity_result(user_token, merged_dict, properties_to_skip, cache_type)
...
def update_entity(id: str, user_token: str, json_data_dict: dict):
    ...
    cache_type = CacheTypeEnum.COMPLETE if len(properties_to_skip) == 0 else None
    complete_dict = schema_manager.get_complete_entity_result(user_token, merged_updated_dict, properties_to_skip, cache_type)
...
def get_ancestors(id):
    ...
    # Return all the details if no property filtering
        ...
        complete_entities_list = schema_manager.get_complete_entities_list(token, ancestors_list, properties_to_skip, CacheTypeEnum.ANCESTRY)
...
def get_descendants(id):
    ...
    # Return all the details if no property filtering
        ...
        complete_entities_list = schema_manager.get_complete_entities_list(user_token, descendants_list, properties_to_skip, CacheTypeEnum.ANCESTRY)
...
def get_parents(id):
    ...
    # Return all the details if no property filtering
        ...
        complete_entities_list = schema_manager.get_complete_entities_list(token, parents_list, properties_to_skip, CacheTypeEnum.ANCESTRY)
...
def get_children(id):
    ...
    # Return all the details if no property filtering
        ...
        complete_entities_list = schema_manager.get_complete_entities_list(user_token, children_list, properties_to_skip, CacheTypeEnum.ANCESTRY)
...
def get_siblings(id):
    ...
    complete_entities_list = schema_manager.get_complete_entities_list(token, sibling_list, properties_to_skip, None)
    # Final result after normalization
    final_result = schema_manager.normalize_entities_list_for_response(complete_entities_list)
...
def get_tuplets(id):
    ...
    complete_entities_list = schema_manager.get_complete_entities_list(token, tuplet_list, properties_to_skip, None)
    # Final result after normalization
    final_result = schema_manager.normalize_entities_list_for_response(complete_entities_list)
...
def get_previous_revisions(id):
    ...
    # Return all the details if no property filtering
        ...
        for multi_list in descendants_multi_list:
            complete_entities_list = schema_manager.get_complete_entities_list(user_token, multi_list, properties_to_skip, None)
            # Final result after normalization
...
def get_dataset_latest_revision(id):
    ...
    complete_dict = schema_manager.get_complete_entity_result(token, latest_revision_dict, properties_to_skip, None)
...
def get_revisions_list(id):
    ...
    complete_revisions_list = schema_manager.get_complete_entities_list(token, sorted_revisions_list, properties_to_skip, None)
...
#             complete_revision_list = schema_manager.get_complete_entities_list(token, revision, properties_to_skip, None)
...
def multiple_components(user_token: str, json_data_dict: dict):
    ...
    for dataset in dataset_list:
        ...
        # Generate the filtered or complete entity dict to send back
        cache_type = CacheTypeEnum.COMPLETE if len(properties_to_skip) == 0 else None
        complete_dict = schema_manager.get_complete_entity_result(user_token, dataset, properties_to_skip, cache_type)
...
def get_collections(id):
    ...
    # Return all the details if no property filtering
        ...
        complete_entities_list = schema_manager.get_complete_entities_list(token, collection_list, properties_to_skip, None)
...
def get_uploads(id):
    ...
    # Return all the details if no property filtering
        ...
        complete_entities_list = schema_manager.get_complete_entities_list(token, uploads_list, properties_to_skip, None)
