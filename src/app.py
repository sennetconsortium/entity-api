...
def get_entity_by_id(id):
    ...
    if normalized_entity_type == 'Dataset':
        ...
        # Otherwise, we should send back the `next_revision_uuid` (if exists) when the member belongs to SenNet-Read group
        if entity_dict['status'].lower() == DATASET_STATUS_PUBLISHED:
...
is provided with group membership in the SenNet-Read group any collection matching the id will be returned.
otherwise if no token is provided or a valid token with no SenNet-Read group membership then
...
"""
Update the properties of a given activity, primarily the protocol_url
...
Request Body
-------
Requires a json in the request body matching the following example
Example:
            {
<required>      "ancestor": {
...
def validate_constraints():
    ...
    if "ancestor" not in json_data_dict or "descendant" not in json_data_dict:
        bad_request_error("Request Json must contain 'ancestor' and 'descendant'")

    ancestor = json_data_dict['ancestor']
    descendant = json_data_dict['descendant']
