...
def build_all_collection_constraints(entity):
    ...
    return [
        build_constraint([ancestor, ancestor2, ancestor3], [descendant])
    ]
...
def build_collection_search_constraints(entity):
    ...
    ancestor2 = build_search_constraint_unit('entity_type.keyword', Ontology.ops().entities().SAMPLE)
    ancestor3 = build_search_constraint_unit('entity_type.keyword', Ontology.ops().entities().SOURCE)
