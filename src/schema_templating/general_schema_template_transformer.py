...
def output_to_yaml():
    try:
        yaml.safe_dump(
            outputyaml, sys.stdout, allow_unicode=True, default_flow_style=False, sort_keys=False
        )
    except Exception as e:
        sys.stderr.write(str(e))
        sys.exit()
...
if __name__ == "__main__":
    ...
    storagedict = (
        {}
    )  # Instantiates a global dictionary. This provides temporary storage of dictionary elements to be used with create_new_yaml function
    secondstorage = (
        {}
    )  # Instantiates a second global dictionary. This is only used for collecting enum lists to use at earlier loops through the recursive function
